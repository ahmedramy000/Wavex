...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // ── تأكد إن wvx_posts موجود (فاضي لو مش موجود) ──
...
function renderFeed() {
  ...
  container.innerHTML = '';
  if (!posts.length) {
    container.innerHTML = `<div class="empty-state-full"><div style="font-size:3rem">📝</div><p style="color:var(--text-muted);margin-top:8px">${currentLang==='ar'?'لا توجد منشورات بعد':'No posts yet'}</p></div>`;
    return;
  }
...
function buildPostCard(post) {
  ...
  let postMediaHTML = '';
  if (post.image) {
    postMediaHTML = '<img src="' + post.image + '" class="post-image" onclick="openLightbox(this.src)" alt=""/>';
  } else if (post.video) {
    postMediaHTML =
...
function renderProfilePosts() {
  ...
  updateProfilePostCount();
  if (!posts.length) {
    grid.innerHTML = `<div class="empty-state-full"><div style="font-size:2.5rem">📝</div><p style="color:var(--text-muted);margin-top:8px">${currentLang==='ar'?'لا توجد منشورات بعد':'No posts yet'}</p></div>`;
    return;
  }
...
function sendMessage(userId) {
  ...
  if (user) simulateReply(userId, key);
}

function simulateReply(userId, key) {
  const delay = 1500 + Math.random() * 2000;
  const replies_ar = ['أهلاً! 😊','كيف حالك؟','تمام جداً 👍','ممتاز!','شكراً لك 🙏','هههه 😂','أيوه بالظبط!','إن شاء الله 🌟','جميل!','صح كلامك 💯'];
  const replies_en = ['Hey! 😊','How are you?','Sounds great 👍','Amazing!','Thanks! 🙏','haha 😂','Exactly!','Sure thing 🌟','Nice!','You\'re right 💯'];
  const replies = currentLang === 'ar' ? replies_ar : replies_en;
  setTimeout(() => {
    if (activeChatId !== userId) return;
    const showTyping = document.getElementById('chat-status');
    if (showTyping) { showTyping.textContent = t('typing'); showTyping.classList.add('typing'); }
    setTimeout(() => {
      const msgs = DB.get(key) || [];
      msgs.push({ id: Date.now(), from: userId, text: replies[Math.floor(Math.random()*replies.length)], time: new Date().toISOString(), read: true });
      DB.set(key, msgs);
      if (showTyping) { showTyping.textContent = t('online'); showTyping.classList.remove('typing'); }
      renderChatMessages(userId);
      updateConvInList(userId, msgs[msgs.length-1].text);
    }, 1200);
  }, delay);
}
...
function renderNotifications() {
  ...
  const isAr  = currentLang === 'ar';
  list.innerHTML = '';
  const icons = { like:'❤️', comment:'💬', follow:'👤', share:'🔁', verified:'🛡️', reaction:'🎉', mention:'📢' };

  // Real user notifications (newest first)
  const userNotifs = (DB.get('wvx_notifs_' + ME?.id) || []).slice().reverse();
  const seedNotifs = NOTIF_SEED.map(n => ({ ...n, text: isAr ? n.text_ar : n.text_en, seed: true }));
  const allNotifs  = [...userNotifs, ...seedNotifs];

  if (!allNotifs.length) {
    list.innerHTML = `<div class="empty-state-full"><div style="font-size:2.5rem">🔔</div><p style="color:var(--text-muted)">${isAr?'لا توجد إشعارات':'No notifications'}</p></div>`;
    return;
  }
  ...
  allNotifs.forEach((n, i) => {
    const d   = document.createElement('div');
    const isNew = !n.seed && i < 3;
    d.className = 'notif-item glass-card' + (isNew ? ' notif-new' : '');
    ...
    const avHTML  = avIsImg
      ...
      : `<div class="notif-avatar">${n.icon||icons[n.type]||'🔔'}</div>`;

    d.innerHTML = `
      ${avHTML}
      <div class="notif-body" style="flex:1;min-width:0">
        <div class="notif-text">${esc(n.text||'')}</div>
        <div class="notif-time">${timeAgo(n.time||n.createdAt||new Date().toISOString())}</div>
      </div>
      ${isNew ? '<div class="notif-new-dot"></div>' : ''}
    `;

    // Click → go to post if exists
    if (n.postId) {
...
function openUserProfile(userId) {
  ...
  openModal(
    ...
    // Cover
    '<div class="up-cover" style="' + coverStyle + '" ' + coverClick + '></div>' +

    // Avatar over cover
    '<div class="up-avatar-wrap">' + avHTML + '</div>' +

    // Follow button
    '<div class="up-follow-row">' +
      '<button class="btn-follow up-follow-btn' + (isFollowing ? ' following' : '') + '" onclick="toggleFollowModal(\'' + userId + '\', this)">' +
        (isFollowing ? '✓ ' + (isAr ? 'يتابع' : 'Following') : '+ ' + (isAr ? 'متابعة' : 'Follow')) +
      '</button>' +
      '<button class="btn-message-sm" onclick="closeModal();startChatWith(\'' + userId + '\')">' +
        '💬 ' + (isAr ? 'رسالة' : 'Message') +
      '</button>' +
    '</div>' +
    ...
    // Info
    '<div class="up-info">' +
      '<div class="up-name">' + esc(user.name) + (user.verified ? ' <span class="badge-verified">✓</span>' : '') + '</div>' +
      '<div class="up-handle">@' + esc(user.username) + '</div>' +
      (user.bio ? '<div class="up-bio">' + esc(user.bio) + '</div>' : '') +
      '<div class="up-stats">' +
        '<div class="up-stat"><span class="up-stat-n">' + userPosts.length + '</span><span class="up-stat-l">' + (isAr ? 'منشور' : 'Posts') + '</span></div>' +
        '<div class="up-stat"><span class="up-stat-n">' + (user.followers || 0) + '</span><span class="up-stat-l">' + (isAr ? 'متابع' : 'Followers') + '</span></div>' +
        '<div class="up-stat"><span class="up-stat-n">' + (user.following || 0) + '</span><span class="up-stat-l">' + (isAr ? 'يتابع' : 'Following') + '</span></div>' +
      '</div>' +
    '</div>' +

    // Recent posts
    (userPosts.length ? '<div class="up-posts-title">' + (isAr ? 'المنشورات الأخيرة' : 'Recent Posts') + '</div>' : '') +
    '</div>'
  );

  // Inject recent posts after modal opens
  setTimeout(() => {
    const modal = document.querySelector('.user-profile-modal');
    if (!modal) return;
    userPosts.slice(0,3).forEach(p => modal.appendChild(buildPostCard(p)));
    // Style modal for profile view
    const mbox = document.getElementById('modal-box');
    if (mbox) { mbox.style.padding = '0'; mbox.style.overflow = 'hidden'; }
    const mc = document.getElementById('modal-content');
