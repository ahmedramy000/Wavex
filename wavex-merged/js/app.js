...
function showFriendsPage() {
  ...
  openModal(`
    <div class="modal-title">👥 ${isAr ? t('friends') : 'Friends'}</div>
...
// All available pages
const ALL_NAV_PAGES = [
  { id:'feed',          icon:'🏠', ar:'الرئيسية',   en:'Home'        },
  { id:'explore',       icon:'🔍', ar:'بحث',         en:'Search'      },
  { id:'chat',          icon:'💬', ar:'الرسايل',     en:'Messages'    },
  { id:'notifications', icon:'🔔', ar:'الإشعارات',   en:'Notifications'},
  { id:'profile',       icon:'👤', ar:'بروفايلي',    en:'Profile'     },
  { id:'market',        icon:'🛒', ar:'الماركت',     en:'Market'      },
  { id:'videos',        icon:'🎬', ar:'الفيديوهات',  en:'Videos'      },
  { id:'groups',        icon:'👥', ar:'الجروبات',    en:'Groups'      },
  { id:'saved',         icon:'🔖', ar:'المحفوظات',   en:'Saved'       },
  { id:'settings',      icon:'⚙️', ar:'الإعدادات',   en:'Settings'    },
];

// Default 4 slots (+ is always center, can't be changed)
const DEFAULT_NAV = ['feed','explore','profile','more'];

function getNavPrefs() {
  return DB.get('wvx_nav_prefs') || DEFAULT_NAV;
}

function saveNavPrefs(slots) {
  DB.set('wvx_nav_prefs', slots);
}

// Apply saved nav to DOM
function applyNavPrefs() {
  const slots  = getNavPrefs();
  const nav    = document.getElementById('mobile-bottom-nav');
  if (!nav) return;
  const isAr   = currentLang === 'ar';

  // Rebuild nav items (keep center + button)
  const createBtn = nav.querySelector('.mbn-create');
  nav.innerHTML = '';

  // Left 2 slots
  [slots[0], slots[1]].forEach(pageId => {
    const page = ALL_NAV_PAGES.find(p => p.id === pageId);
    if (!page) return;
    const a = document.createElement('a');
    a.className = 'mbn-item';
    a.id = 'mbn-' + page.id;
    a.onclick = () => {
      if (page.id === 'more') toggleMoreMenu();
      else showPage(page.id);
    };
    a.innerHTML = `<span class="mbn-icon">${page.icon}</span><span class="mbn-label">${isAr ? page.ar : page.en}</span>`;
    nav.appendChild(a);
  });
  ...
  if (createBtn) {
    ...
    c.className = 'mbn-create';
    c.onclick = toggleCreateMenu;
    c.innerHTML = `<div class="mbn-create-btn" id="mbn-create-btn"><span id="mbn-create-icon">＋</span></div>`;
    nav.appendChild(c);
  }

  // Right 2 slots
  [slots[2], slots[3]].forEach(pageId => {
    const page = ALL_NAV_PAGES.find(p => p.id === pageId) || { id:'more', icon:'☰', ar:'المزيد', en:'More' };
    const a = document.createElement('a');
    a.className = 'mbn-item';
    a.id = 'mbn-' + (page.id === 'more' ? 'more' : page.id);
    a.onclick = () => {
      if (page.id === 'more') toggleMoreMenu();
      else showPage(page.id);
    };
  ...
  // Sync active state
  syncMobileNav(document.querySelector('.page.active')?.id?.replace('page-','') || 'feed');
}

function showNavCustomizer() {
  const isAr    = currentLang === 'ar';
  const slots   = getNavPrefs();
  const slotNames = slots.map(id => {
    if (id === 'more') return isAr ? '☰ المزيد' : '☰ More';
    const p = ALL_NAV_PAGES.find(x => x.id === id);
    return p ? p.icon + ' ' + (isAr ? p.ar : p.en) : id;
  });

  openModal(`
    <div class="modal-title">📱 ${isAr ? 'تخصيص الشريط السفلي' : 'Customize Bottom Bar'}</div>
    <p style="color:var(--text-muted);font-size:.8rem;margin-bottom:16px">
      ${isAr ? 'اختار 4 صفحات تظهر في الشريط السفلي (زرار + دايماً في المنتصف)' : 'Choose 4 tabs for the bottom bar (+ is always center)'}
    </p>

    <!-- Current slots preview -->
    <div class="nav-preview" id="nav-preview">
      <div class="nav-preview-bar">
        ${[0,1,'plus',2,3].map(i => i === 'plus'
          ? '<div class="npb-item npb-plus"><span>＋</span></div>'
          : `<div class="npb-item" id="npb-slot-${i}" onclick="startNavSlotEdit(${i})">
              <span style="font-size:1.2rem">${(ALL_NAV_PAGES.find(p=>p.id===slots[i])||{icon:'☰'}).icon}</span>
              <span style="font-size:.6rem;margin-top:2px">${slotNames[i]?.split(' ').pop()||'?'}</span>
              <span class="npb-edit-dot">✏️</span>
             </div>`
        ).join('')}
      </div>
      <p style="font-size:.72rem;color:var(--text-muted);text-align:center;margin-top:8px">
        ${isAr ? 'اضغط على أي زرار لتغييره' : 'Tap any tab to change it'}
      </p>
    </div>

    <!-- Page selector (hidden until slot chosen) -->
    <div id="nav-page-selector" class="hidden">
      <p style="font-size:.82rem;font-weight:700;margin-bottom:10px" id="nav-select-hint"></p>
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px">
        ${ALL_NAV_PAGES.map(p => `
          <div class="nav-page-opt" onclick="setNavSlot(window._editingNavSlot,'${p.id}')" id="navopt-${p.id}"
            style="display:flex;flex-direction:column;align-items:center;gap:5px;padding:12px 8px;
            background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);
            border-radius:14px;cursor:pointer;transition:all .18s;font-size:.78rem;color:var(--text-sub)">
    ...
    <button class="btn-primary" onclick="closeModal();applyNavPrefs()" style="margin-top:14px">
      ${isAr ? 'حفظ' : 'Save'}
    </button>
  `);

  window._currentNavSlots = [...slots];
}

function startNavSlotEdit(slotIdx) {
  const isAr = currentLang === 'ar';
  window._editingNavSlot = slotIdx;
  const selector = document.getElementById('nav-page-selector');
  const hint     = document.getElementById('nav-select-hint');
  if (selector) selector.classList.remove('hidden');
  if (hint) hint.textContent = isAr
    ? `اختار صفحة للمكان ${slotIdx + 1}`
    : `Choose page for slot ${slotIdx + 1}`;

  // Highlight current
  document.querySelectorAll('.nav-page-opt').forEach(el => el.style.background = 'rgba(255,255,255,.05)');
  const current = window._currentNavSlots[slotIdx];
  const curEl = document.getElementById('navopt-' + current);
  if (curEl) curEl.style.background = 'rgba(110,231,247,0.15)';
}

function setNavSlot(slotIdx, pageId) {
  const isAr = currentLang === 'ar';
  if (slotIdx === undefined || slotIdx === null) return;
  window._currentNavSlots[slotIdx] = pageId;
  saveNavPrefs(window._currentNavSlots);

  // Update preview bar
  const page = ALL_NAV_PAGES.find(p => p.id === pageId);
  const slotEl = document.getElementById('npb-slot-' + slotIdx);
  if (slotEl && page) {
    slotEl.innerHTML = `
      <span style="font-size:1.2rem">${page.icon}</span>
      <span style="font-size:.6rem;margin-top:2px">${isAr ? page.ar : page.en}</span>
      <span class="npb-edit-dot">✏️</span>`;
  }

  // Highlight selected option
  document.querySelectorAll('.nav-page-opt').forEach(el => el.style.background = 'rgba(255,255,255,.05)');
  const optEl = document.getElementById('navopt-' + pageId);
  if (optEl) optEl.style.background = 'rgba(110,231,247,0.15)';
  if (optEl) optEl.style.borderColor = 'rgba(110,231,247,0.35)';

  showToast(isAr ? 'تم التغيير ✅' : 'Changed ✅', '✅');
}
